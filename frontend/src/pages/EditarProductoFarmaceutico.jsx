...
function EditarProductoFarmaceutico() {
  const { id } = useParams();
  ...
  const fetchProducto = async () => {
    try {
      ...
      const res = await axios.get(`/api/productos/farmaceuticos/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/api/productos/farmaceuticos/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="animate-fadeIn max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Editar Producto Farmacéutico</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6 lg:p-8">
          ...
          <div className="mt-6">
            <label className="block text-sm font-medium text-slate-700 mb-2">PAVS</label>
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input
                type="checkbox"
                name="pavs"
...
export default EditarProductoFarmaceutico;