...
function EditarProductoBiologico() {
  const { id } = useParams();
  ...
  const fetchProducto = async () => {
    try {
      ...
      const res = await axios.get(`/api/productos/biologicos/${id}`, {
      ...
      setFormData({
        ...
        vacunas_inmunologicos: Boolean(data.vacunas_inmunologicos),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/api/productos/biologicos/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 font-medium">Cargando datos del producto...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fadeIn max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Editar Producto Biológico</h1>
          <p className="text-slate-500 text-sm mt-1">Modifica los datos del producto</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6 lg:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Código de Registro</label>
              <input
                type="text"
          ...
          {/* PAVS (checkbox) - en edición usamos checkbox simple */}
          <div className="mt-6">
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input
                type="checkbox"
                name="pavs"
                checked={formData.pavs}
                onChange={handleChange}
                className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500"
              />
              ¿Tiene PAVS?
...
export default EditarProductoBiologico;